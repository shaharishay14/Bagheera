...
export default function InferencePage() {
  ...
  useEffect(() => {
    if (!modelId || selectedPaths.length === 0) { setCacheStates({}); return; }
    let cancelled = false;
    setCacheStates((prev) => {
      const next: Record<string, CacheState> = { ...prev };
      for (const p of selectedPaths) { if (!(p in next)) next[p] = 'pending'; }
      for (const k of Object.keys(next)) { if (!selectedPaths.includes(k)) delete next[k]; }
      return next;
    });
    (async () => {
      for (const p of selectedPaths) {
        try {
          ...
          if (cancelled) return;
          setCacheStates((prev) => ({ ...prev, [p]: hit ? { hit } : { miss: true } }));
        } catch {
          if (cancelled) return;
  ...
  useEffect(() => {
    if (trackingIds.length === 0) { setTrackedInferences([]); return; }
    let cancelled = false;
    const poll = async () => {
      try {
        ...
        const allDone = fetched.every((inf) => inf.status === 'ready' || inf.status === 'failed');
  ...
  const aggregateCacheCounts = useMemo(() => {
    let cached = 0, willProcess = 0, pending = 0;
    for (const p of selectedPaths) {
      const s = cacheStates[p];
      if (!s || s === 'pending') pending += 1;
      else if ('hit' in s) { if (rerunSet.has(p)) willProcess += 1; else cached += 1; }
      else willProcess += 1;
    }
    ...
    try {
      ...
      if (needsRerunFlag) {
        ...
        if (trueMisses.length > 0) {
          const res = await createInferences({ model_id: modelId, wsi_paths: trueMisses });
          for (const e of res.inferences) { if (e.id) ids.push(e.id); }
        }
      } else {
        const res = await createInferences({ model_id: modelId, wsi_paths: pathsToSend });
        for (const e of res.inferences) { if (e.id) ids.push(e.id); }
      }
      ...
      <Card className="p-4">
        ...
        <div className="mt-4">
          <button
            ...
            className="rounded-md border border-border-strong bg-surface px-3 py-1.5 text-sm text-ink hover:bg-surface-subtle transition-colors"
          >
        ...
        {submitError ? (
          <div className="mt-3 rounded-md border border-[var(--s-failed-border)] bg-[var(--s-failed-bg)] px-3 py-2 text-xs text-[var(--s-failed-text)]">
            {submitError}
          </div>
        ) : null}
        ...
        <button
          ...
          className="mt-4 w-full rounded-full bg-grad-accent px-4 py-2 text-sm font-semibold text-white shadow-glow hover:shadow-glow-lg hover:brightness-105 disabled:cursor-not-allowed disabled:opacity-50 disabled:shadow-none transition-all duration-150"
        >
          {submitting
            ? 'Queuing…'
            : mode === 'single'
...
function ParamsCard({ params }: { params: TridentParamsResponse }) {
  if (!params.patch_encoder) {
    return (
      <div className="rounded-md border border-[var(--s-warn-border)] bg-[var(--s-warn-bg)] px-4 py-3 text-sm text-[var(--s-warn-text)]">
  ...
  return (
    <div className="rounded-md border border-border bg-surface-subtle px-4 py-3 text-sm text-ink">
      <p className="font-semibold text-ink">This model expects:</p>
      <dl className="mt-1 grid grid-cols-[140px_1fr] gap-x-3 gap-y-1 font-mono text-xs">
        <dt className="text-ink-faint">Patch encoder</dt>
        <dd>{params.patch_encoder}</dd>
        <dt className="text-ink-faint">Magnification</dt>
        <dd>{params.mag}×</dd>
        <dt className="text-ink-faint">Patch size</dt>
...
function HistorySection({
  open, onToggle, history, onOpen,
}: {
  open: boolean; onToggle: () => void;
  history: InferenceInfo[]; onOpen: (inf: InferenceInfo) => void;
}) {
  return (
    <Card>
      <button
      ...
      {open ? (
        history.length === 0 ? (
          ...
          <ul className="divide-y divide-border border-t border-border">
            {history.map((inf) => (
              <li key={inf.id} className="flex items-center justify-between px-4 py-2 text-xs hover:bg-surface-subtle">
                <button
                  type="button"
                  onClick={() => onOpen(inf)}
                  className="flex-1 truncate text-left font-mono text-ink-muted hover:text-ink"
                >
                  {inf.wsi_filename}
                </button>
...
function SelectionList({
  paths, cacheStates, rerunSet, onToggleRerun,
}: {
  paths: string[]; cacheStates: Record<string, CacheState>;
  rerunSet: Set<string>; onToggleRerun: (path: string) => void;
}) {
  return (
      ...
      {paths.map((p) => {
        ...
        return (
          <li key={p} className="flex items-center justify-between px-3 py-2 text-xs">
            <span className="flex-1 truncate font-mono text-ink-muted" title={p}>{basename}</span>
            <span className="ml-3 shrink-0">
              {!s || s === 'pending' ? (
                <span className="text-ink-faint">checking…</span>
              ) : 'hit' in s ? (
                <span className="flex items-center gap-2">
                  <span className="text-[var(--s-success-text)]">✓ Cached</span>
                ...
                <span className="text-ink-muted">○ Will be processed</span>
              )}
            </span>
          </li>
        );
...
function ResultsSection({
  mode, model, inferences, activeId, onActive,
}: {
  mode: Mode; model: ModelInfo; inferences: InferenceInfo[];
  activeId: string | null; onActive: (id: string) => void;
}) {
    ...
    return (
      <div className="space-y-3">
        <div className="rounded-md border border-border bg-surface-subtle px-4 py-3 text-sm">
          <p className="font-semibold text-ink">Batch progress</p>
          ...
          {inferences.map((inf) => (
            <li
              ...
              className={`flex items-center gap-3 px-3 py-2 text-xs transition-colors ${
                activeId === inf.id ? 'bg-surface-subtle' : 'hover:bg-surface-subtle'
              }`}
            >
              <button
                type="button"
              ...
              <StatusPill status={inf.status} />
              {inf.error_message ? (
                <span className="text-[var(--s-failed-text)]" title={inf.error_message}>⚠</span>
              ) : null}
            </li>
          ))}
        </ul>
...
function SingleResultView({ inference, model }: { inference: InferenceInfo; model: ModelInfo }) {
  const [tsneOpen, setTsneOpen] = useState(false);

  if (inference.status === 'failed') {
    return (
      <div className="rounded-md border border-[var(--s-failed-border)] bg-[var(--s-failed-bg)] px-4 py-3 text-sm text-[var(--s-failed-text)]">
        <p className="font-semibold">Inference failed for {inference.wsi_filename}</p>
  ...
  if (inference.status !== 'ready') {
    return (
      <div className="rounded-md border border-[var(--s-running-border)] bg-[var(--s-running-bg)] px-4 py-3 text-sm text-[var(--s-running-text)]">
        <p>{inference.wsi_filename}: <StatusPill status={inference.status} /></p>
  ...
  return (
    <Card className="space-y-5 p-4">
      <div>
        <p className="text-xs text-ink-faint">Slide</p>
        <p className="font-mono text-sm text-ink">{inference.wsi_filename}</p>
      </div>
      ...
      <section>
        ...
        <img
          ...
          alt="Mixture plot"
          className="mt-2 w-full max-w-xl rounded border border-border bg-surface"
        />
      </section>
      ...
      <section>
        ...
        >
          ...
          <span className="text-xs text-ink-faint">{tsneOpen ? 'Hide' : 'Show'}</span>
        </button>
        {tsneOpen ? (
          <img
            src={resolveVizUrl(
...
function ExamplePatchesGrid({ inferenceId, nProto }: { inferenceId: string; nProto: number }) {
  ...
  if (loading) return <p className="mt-2 text-xs text-ink-faint">Loading patches…</p>;
  if (error) {
    return (
      <p className="mt-2 rounded border border-[var(--s-failed-border)] bg-[var(--s-failed-bg)] px-3 py-2 text-xs text-[var(--s-failed-text)]">
        {error}
      </p>
    );
  }
      ...
      <div className="mt-2">
        <img
          src={vizPlaceholderUrl('patches', {
            label: `${nProto} × example patches`, width: 720, height: 320,
          })}
          alt="Example patches"
          ...
          <tbody>
            {data.groups.map((g) => (
              <tr key={g.prototype_index}>
                <td className="pr-2 text-right align-middle text-[11px] text-ink-faint whitespace-nowrap">
                  <div className="font-mono text-ink-muted">P{g.prototype_index}</div>
                  ...
                  <td key={i} className="align-middle">
                    ...
                    >
                      <img
                        ...
                        className="h-16 w-16 cursor-zoom-in rounded border border-border bg-surface-subtle object-cover hover:border-accent transition-colors"
                      />
                    </button>
                  </td>
                ))}
...
function Lightbox({ src, onClose }: { src: string; onClose: () => void }) {
  ...
  return createPortal(
    <div
      role="presentation"
      onClick={onClose}
      className="fixed inset-0 z-50 flex items-center justify-center bg-ink/75 p-6 backdrop-blur-sm"
    >
      <img
        src={src}
        alt="Patch"
        onClick={(e) => e.stopPropagation()}
        className="max-h-full max-w-full rounded shadow-modal"
      />
      <button
        type="button"
        onClick={onClose}
        className="absolute right-4 top-4 rounded-full bg-surface/90 px-3 py-1 text-sm font-semibold text-ink shadow hover:bg-surface transition-colors"
      >
        Close ✕
      </button>
    </div>,
...
function DatasetReferenceTile({ label, src }: { label: string; src: string }) {
  return (
    <div>
      <p className="mb-1 text-[11px] font-semibold uppercase tracking-widest text-ink-faint">{label}</p>
      <img src={src} alt={label} className="w-full rounded border border-border bg-surface" />
    </div>
  );
}
